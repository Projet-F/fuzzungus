...
@deprecated.deprecated(version='1.0', reason='Use FuzzLoggerPostgres instead')
class FuzzLoggerDb(IFuzzLoggerBackend):
    """
    Deprecated if favor of `FuzzLoggerPostgres`.
    ...
    def __init__(self, db_filename, num_log_cases=0):
        ...
        if not db_already_exist:
            self._db_cursor.execute("""CREATE TABLE cases (name text, number integer, round_type text, 
                                                         seed text, seed_index integer, timestamp TEXT)""")
    ...
    def open_test_case(self, test_case_id, name, index, round_type=None, seed=None, seed_index=None, *args,
                       **kwargs):
        self._queue.append(
            ["INSERT INTO cases VALUES(?, ?, ?, ?, ?, ?);\n", name, index, round_type, seed, seed_index,
